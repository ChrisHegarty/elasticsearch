...
EXPORT void dot7u_bulk(int8_t* a, int8_t* b, size_t dims, size_t count, float_t* results) {
    ...
    for (; c + 3 < count; c += 4) {
        const int8_t* a0 = a;
        const int8_t* a1 = a +     dims;
        const int8_t* a2 = a + 2 * dims;
        const int8_t* a3 = a + 3 * dims;
        ...
        if (blk != dims) {
            ...
            for (size_t t = blk; t < dims; t++) {
                const int8_t bb = b[t];
                acc_scalar0 += (int32_t)a0[t] * bb;
                acc_scalar1 += (int32_t)a1[t] * bb;
                acc_scalar2 += (int32_t)a2[t] * bb;
                acc_scalar3 += (int32_t)a3[t] * bb;
            }
        }
        results[c + 0] = (float_t)acc_scalar0;
        results[c + 1] = (float_t)acc_scalar1;
        results[c + 2] = (float_t)acc_scalar2;
        results[c + 3] = (float_t)acc_scalar3;
    ...
    for (; c < count; c++) {
        results[c] = (float)dot7u(a, b, dims);
        a += dims;
    }
}
