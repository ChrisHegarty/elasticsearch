...
public class FrozenFieldNameTableTests extends ESTestCase {
    ...
    // lookupField carries the ordinal assigned at freeze time, which keys the caller's per-field caches.
    ...
    public void testPrematureDeferredFreezeStillCorrect() {
        ...
        // The variantB pass above inserted "score", so the overflow buffer canonicalizes it from
        // here on: the lookup hits, but with no dense ordinal because the frozen table lacks it.
        byte[] scoreBuf = toBytes("score");
        int scoreLen = "score".length();
        int scoreHash = FieldNameHash.hashName(scoreBuf, 0, scoreLen);
        ResolvedFieldName score = child.lookupField(scoreBuf, 0, scoreLen, scoreHash, FieldNameHash.readPrefix8(scoreBuf, 0, scoreLen));
        assertNotNull("overflow must resolve a name learned after freeze", score);
        assertEquals("score", score.name());
        assertEquals("an overflow name has no dense ordinal", -1, score.ordinal());
        assertSame(
            "re-resolving must reuse the instance rather than allocate",
            score.name(),
            child.lookup(scoreBuf, 0, scoreLen, scoreHash)
        );
    }
