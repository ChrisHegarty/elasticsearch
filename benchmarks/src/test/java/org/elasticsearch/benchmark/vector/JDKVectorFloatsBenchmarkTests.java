...
public class JDKVectorFloatsBenchmarkTests extends ESTestCase {

    final double delta = 1e-3;
    final int size;

    public JDKVectorFloatsBenchmarkTests(int size) {
        this.size = size;
    }
    ...
    public void testCosine() {
        for (int i = 0; i < 100; i++) {
            var bench = new JDKVectorFloatsBenchmark();
            bench.size = size;
            bench.init();
            try {
    ...
    public static Iterable<Object[]> parametersFactory() {
        try {
            var params = JDKVectorFloatsBenchmark.class.getField("size").getAnnotationsByType(Param.class)[0].value();
